...
public class GameAGui extends Applet implements Runnable {
	...
	public void paint(Graphics g) {
		if (game.move == -1) {
			...
			for (int r = 0; r < GameA.SQUARES_PER_SIDE; r++) {
				for (int c = 0; c < GameA.SQUARES_PER_SIDE; c++) {
					if (game.gameBoard[0][r * GameA.SQUARES_PER_SIDE + c] == GameA.X_MARK)
						g.drawImage(xMark, c * 30, r * 30 + 40, this);
					else if (game.gameBoard[0][r * GameA.SQUARES_PER_SIDE
							+ c] == GameA.ZERO_MARK) {
						if (r * GameA.SQUARES_PER_SIDE + c == game.lastMove)
		...
		if (game.gameState == 0)
			g.drawImage(yourTurnImage, GameA.TOTAL_SQUARES_PER_BOARD, 0,
					this);
		else if (game.gameState == 2)
			g.drawImage(winningImage, GameA.TOTAL_SQUARES_PER_BOARD, 0,
					this);
		else if (game.gameState == 3)
			g.drawImage(losingImage, GameA.TOTAL_SQUARES_PER_BOARD, 0,
					this);
	}
	...
	public boolean mouseUp(Event evt, int x, int y) {
		...
		if (y < 40) {
			if (x < GameA.TOTAL_SQUARES_PER_BOARD) // new game
			{
				game.run();
				repaint();
			}
		...
		int playerMove = y * GameA.SQUARES_PER_SIDE + x;

		System.out.println("playerMove = " + playerMove);
		System.out.println("player x = " + x);
		System.out.println("player y = " + y);

		if (game.gameBoard[0][playerMove] != GameA.EMPTY
				|| game.gameState != 0 || game.move > 49) 
		{
			return true;
		}
