...
public class GameA {
	...
	public static final int ZERO_MARK = 2;
	...
	public int makeComputerMove(int x, int y, boolean reporting) {
		...
		position = closeGap();
		if ((move > 8) && (position != NONE)) {
			if (reporting)
				System.out.println("closeGap() found " + position);
			return position;
		}

		setFlagsForLaterProcessing(X_MARK);
		position = responseTo3Or4InaRowOpportunity(ZERO_MARK, 0,
				CHECK_MODE);
		if ((move > 6) && (position != NONE)) {
			if (reporting)
		...
		if (move > 15) {
			position = addToSeries(ZERO_MARK, 0,
					CHECK_MODE);
			if (position != NONE) {
				if (reporting)
					System.out.println("addToSeries() found "
							+ position);
				return position;
			}

			position = addToSeries(X_MARK, 0, CHECK_MODE);
			if (position != NONE) {
				if (reporting)
		...
		position = responseTo3Or4InaRowOpportunity(X_MARK, 0,
				CHECK_MODE);
		if ((move > 5) && (position != NONE)) {
			if (reporting)
				System.out.println("responseTo3Or4InaRowOpportunity() found "
						+ position);
			return position;
		...
		setFlagsForLaterProcessing(ZERO_MARK);

		position = tryToMake3WithGap_FromVert4IntersectingWithHoriz4(
				X_MARK, 0);
		if ((move > 4) && (position != NONE)) {
			if (reporting)
				System.out
						.println("tryToMake3WithGap_FromVert4IntersectingWithHoriz4() found "
								+ position);
		...
		position = checkSeries(ZERO_MARK, 0);
		if ((move > 4) && (position != NONE)) {
			if (reporting)
		...
		if (move > 6
				&& (position = checkSeries(X_MARK, 0)) != NONE) {
			if (reporting)
				System.out.println("checkSeries() found " + position);
			return position;
		}

		position = check2o3c(ZERO_MARK, 0);
		if ((move > 3) && (position != NONE)) {
			if (reporting)
				System.out.println("check2o3c() found " + position);
			return position;
		}

		position = check2o3c(X_MARK, 0);
		if ((move > 20) && (position != NONE)) {
			if (reporting)
				System.out.println("check2o3c() found " + position);
			return position;
		}
		...
		if (move > 8
				&& (position = checkCross(X_MARK)) != NONE) {
			if (reporting)
		...
		if (move > 4
				&& (position = checkCross(ZERO_MARK)) != NONE) {
			if (reporting)
				System.out.println("checkCross() found " + position);
			return position;
		}

		if (move > 4 && (position = checkBox(X_MARK)) != NONE) {
			if (reporting)
				System.out.println("checkBox() found " + position);
			return position;
		}

		if (move > 6 && (position = closeGap()) != NONE) {
			if (reporting)
		...
		if ((position = responseTo3Or4InaRowOpportunity(ZERO_MARK,
				0, CLEAN_MODE)) != NONE) {
			if (reporting)
				System.out.println("responseTo3Or4InaRowOpportunity() found "
						+ position);
		...
		if ((position = addToSeries(X_MARK, 0,
				CLEAN_MODE)) != NONE) {
			if (reporting)
	...
	private int createTwoAxesOrCreateOneAndBlockAnother() {
		...
		for (i = 11; i < 89; i++)
			if (stagingBoard[i] == OCCUPIED
					&& (gameBoard[0][i - 11] == X_MARK
							|| gameBoard[0][i - SQUARES_PER_SIDE] == X_MARK
							|| gameBoard[0][i - 9] == X_MARK
							|| gameBoard[0][i - 1] == X_MARK
							|| gameBoard[0][i + 1] == X_MARK
							|| gameBoard[0][i + 9] == X_MARK
							|| gameBoard[0][i + SQUARES_PER_SIDE] == X_MARK || gameBoard[0][i + 11] == X_MARK)) {
	...
	public int checkForWinOpportunity(int playerMark, int x) {
		...
		if ((position = addToSeries(switchPlayers(playerMark), x,
				CHECK_MODE)) != NONE)
			return position;
		...
		for (l = 0; l < 6; l++) {
			for (j = 0; j < 10; j++) {
				...
				for (k = 0; k < 5; k++) {
					...
					marksByAxisByPlayerForChecking[4 + gameBoard[0][l * 10 + j
							+ k * 10]]++;
				}
			...
			for (j = 0; j < 6; j++) {
				for (k = 0; k < 8; k++)
					marksByAxisByPlayerForChecking[k] = 0;
				for (k = 0; k < 5; k++) {
					marksByAxisByPlayerForChecking[gameBoard[0][l * 10 + j + k
							* 11]]++;
					marksByAxisByPlayerForChecking[4 + gameBoard[0][l * 10 + j
							- k * 9 + 40]]++;
				}
	...
	public int checkSeries(int playerMark, int depth) {
		...
		for (int k = 0; k < TOTAL_SQUARES_PER_BOARD; k++) {
			...
			if (addToSeries(playerMark, stagingDepth, CHECK_MODE) != NONE) {
				return k;
			}

			if (addToSeries(switchPlayers(playerMark),
		...
		for (j = 0; j < 10; j++) {
			for (l = 0; l < 6; l++) {
				...
				if (marksByAxisByPlayerForChecking[playerMark] == 4
						&& marksByAxisByPlayerForChecking[GameA.EMPTY] == 1) {
					if (type == GameA.SAFE_MODE) {
						flag2 = GameA.EMPTY;
						for (k = 0; k < 5; k++) {
							if (gameBoard[x][j * 10 + l + k] == GameA.EMPTY
									&& tempTableForChecks[j * 10 + l + k] == GameA.OCCUPIED) {
								flag2 = GameA.OCCUPIED;
							}
						}
						if (flag2 == GameA.EMPTY) {
							zbir++;
							flag = GameA.OCCUPIED;
							break;
						}
			...
			for (l = 0; l < 6; l++) {
				...
				if (marksByAxisByPlayerForChecking[playerMark] == 4
					...
					if (type == GameA.SAFE_MODE) {
						flag2 = GameA.EMPTY;
						for (k = 0; k < 5; k++)
							if (gameBoard[x][l * 10 + j + k * 10] == GameA.EMPTY
									&& tempTableForChecks[l * 10 + j + k * 10] == GameA.OCCUPIED)
								flag2 = GameA.OCCUPIED;
						if (flag2 == GameA.EMPTY) {
							zbir++;
							flag = GameA.OCCUPIED;
			...
			for (j = 0; j < 6; j++) {
				...
				if (marksByAxisByPlayerForChecking[playerMark] == 4
					...
					if (type == GameA.SAFE_MODE) {
						flag2 = GameA.EMPTY;
						for (k = 0; k < 5; k++) {
							if (gameBoard[x][l * 10 + j + k * 11] == GameA.EMPTY
									&& tempTableForChecks[l * 10 + j + k * 11] == GameA.OCCUPIED) {
								flag2 = GameA.OCCUPIED;
							}
						}
						if (flag2 == GameA.EMPTY) {
			...
			for (j = 0; j < 6; j++) {
				...
				if (marksByAxisByPlayerForChecking[playerMark] == 4
					...
					if (type == GameA.SAFE_MODE) {
						flag2 = GameA.EMPTY;
						for (k = 0; k < 5; k++) {
							if (gameBoard[x][l * 10 + j - k * 9 + 40] == GameA.EMPTY
	...
	public int addToSeries(int playerMark, int x, int type) {
		int j, k, l;
	...
	public int closeGap() {
		...
		for (k = 1; k < 7; k++) {
			for (l = 1; l < 9; l++) {
				...
				if (gameBoard[0][x] == X_MARK
						&& gameBoard[0][x + 2] == X_MARK
						&& gameBoard[0][x + 1] == 0)
					return (x + 1);

				if (gameBoard[0][y] == X_MARK
						&& gameBoard[0][y + 20] == X_MARK
	...
	public void setFlagsForLaterProcessing(int playerMark) {
		...
		addToSeries(playerMark, 0, SETFLAGS_MODE);
	...
	public void respondToMouseUp(int playerMove, int x, int y) {
		move++;
		markMove(playerMove, X_MARK);
		if (win() == EMPTY) {
			lastMove = makeComputerMove(x, y, true);
			markMove(lastMove, ZERO_MARK);
			gameState = 0;
		}
		if (win() == ZERO_MARK)
			gameState = 3;
